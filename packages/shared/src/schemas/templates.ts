import { z } from 'zod';
import { TemplateVisibility, TemplateDifficulty, TemplateSeason } from '../enums';
import { MAX_TRIP_NAME_LENGTH, MAX_DESCRIPTION_LENGTH, CURRENCIES } from '../constants';
...
export const createTemplateSchema = z.object({
  ...
  estimatedBudgetBreakdown: z.object({
...
export const cloneTemplateSchema = z.object({
  ...
  (data) => data.existingTripId || (data.tripName && data.startDate && data.endDate),
...
export const updateTemplateEventSchema = z.object({
  title: z.string().min(1).max(MAX_TRIP_NAME_LENGTH),
